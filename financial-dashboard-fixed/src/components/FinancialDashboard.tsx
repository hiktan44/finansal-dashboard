...
const FinancialDashboard = () => {
  ...
  const averageIndexChange = marketData.indices?.indices?.reduce((sum: number, idx: any) => 
    sum + idx.changePercent, 0
  ) / (marketData.indices?.indices?.length || 1)
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className={`text-sm ${averageIndexChange >= 0 ? 'text-green-100' : 'text-red-100'}`}>
                  Ort. {averageIndexChange >= 0 ? '+' : ''}{averageIndexChange.toFixed(2)}%
                </p>
              </div>
